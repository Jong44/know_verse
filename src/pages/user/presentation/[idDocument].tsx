...
const Index = () => {
    ...
    const fetchDocument = async () => {
      try {
        ...
        if (data.status === 'success') {
          const flter = data.data.tutorial_detail.filter((item: any) => item.status === "visible");
          const afterFilter =  {
            ...data.data,
            detail_document: flter.map((item: any) => item.content_json)
          }
          setDocument(afterFilter);
        } else {
  ...
  return (
    <>
      <Head>
        <title>Presentation - {document?.judul}</title>
